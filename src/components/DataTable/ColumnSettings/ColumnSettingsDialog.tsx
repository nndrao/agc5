  ...
  const handleApply = () => {
    ...
    try {
      ...
      const updatedColumnDefs = localColumnState.map(colState => {
        ...
        // In AG-Grid 33+, we need to use headerClass and headerComponentParams instead of headerStyle
        // Create a CSS class for the header styling
        const headerStyleClass = `header-style-${colState.colId}`;
        let hasHeaderStyle = false;
        let headerStyleRules = '';

        // Build CSS rules for the header style
        if (colState.headerAlignment !== undefined) {
          headerStyleRules += `text-align: ${colState.headerAlignment}; `;
          hasHeaderStyle = true;
        }

        if (colState.headerBackgroundColor !== undefined) {
          headerStyleRules += `background-color: ${colState.headerBackgroundColor}; `;
          hasHeaderStyle = true;
        }

        if (colState.headerTextColor !== undefined) {
          headerStyleRules += `color: ${colState.headerTextColor}; `;
          hasHeaderStyle = true;
        }

        if (colState.headerFontFamily !== undefined) {
          headerStyleRules += `font-family: ${colState.headerFontFamily}; `;
          hasHeaderStyle = true;
        }

        if (colState.headerFontSize !== undefined) {
          headerStyleRules += `font-size: ${typeof colState.headerFontSize === 'number' ? `${colState.headerFontSize}px` : colState.headerFontSize}; `;
          hasHeaderStyle = true;
        }

        if (colState.headerFontWeight !== undefined) {
          headerStyleRules += `font-weight: ${colState.headerFontWeight}; `;
          hasHeaderStyle = true;
        }

        if (colState.headerFontStyle !== undefined) {
          headerStyleRules += `font-style: ${colState.headerFontStyle}; `;
          hasHeaderStyle = true;
        }

        // Add header border styles
        if (colState.headerBorderTop && colState.headerBorderTop.style !== 'none') {
          headerStyleRules += `border-top: ${colState.headerBorderTop.width || '1px'} ${colState.headerBorderTop.style || 'solid'} ${colState.headerBorderTop.color || '#dddddd'}; `;
          hasHeaderStyle = true;
        }

        if (colState.headerBorderRight && colState.headerBorderRight.style !== 'none') {
          headerStyleRules += `border-right: ${colState.headerBorderRight.width || '1px'} ${colState.headerBorderRight.style || 'solid'} ${colState.headerBorderRight.color || '#dddddd'}; `;
          hasHeaderStyle = true;
        }

        if (colState.headerBorderBottom && colState.headerBorderBottom.style !== 'none') {
          headerStyleRules += `border-bottom: ${colState.headerBorderBottom.width || '1px'} ${colState.headerBorderBottom.style || 'solid'} ${colState.headerBorderBottom.color || '#dddddd'}; `;
          hasHeaderStyle = true;
        }

        if (colState.headerBorderLeft && colState.headerBorderLeft.style !== 'none') {
          headerStyleRules += `border-left: ${colState.headerBorderLeft.width || '1px'} ${colState.headerBorderLeft.style || 'solid'} ${colState.headerBorderLeft.color || '#dddddd'}; `;
          hasHeaderStyle = true;
        }

        // Only set headerClass if we have properties to apply
        if (hasHeaderStyle) {
          // Add the CSS style to the document
          const styleId = `header-style-${colState.colId}`;
          ...
          styleElement.textContent = `.${headerStyleClass} { ${headerStyleRules} }`;

          // Set the headerClass property
          updatedColDef.headerClass = headerStyleClass;
        } else {
          // If no header style properties are defined, remove the headerClass property
          delete updatedColDef.headerClass;
        }

        // In AG-Grid 33+, we can use cellClass for styling cells
        // Create a CSS class for the cell styling
        const cellStyleClass = `cell-style-${colState.colId}`;
        let hasCellStyle = false;
        let cellStyleRules = '';

        // Build CSS rules for the cell style
        if (colState.cellAlignment !== undefined) {
          cellStyleRules += `text-align: ${colState.cellAlignment}; `;
          hasCellStyle = true;
        }

        if (colState.cellBackgroundColor !== undefined) {
          cellStyleRules += `background-color: ${colState.cellBackgroundColor}; `;
          hasCellStyle = true;
        }

        if (colState.cellTextColor !== undefined) {
          cellStyleRules += `color: ${colState.cellTextColor}; `;
          hasCellStyle = true;
        }

        if (colState.cellFontFamily !== undefined) {
          cellStyleRules += `font-family: ${colState.cellFontFamily}; `;
          hasCellStyle = true;
        }

        if (colState.cellFontSize !== undefined) {
          cellStyleRules += `font-size: ${typeof colState.cellFontSize === 'number' ? `${colState.cellFontSize}px` : colState.cellFontSize}; `;
          hasCellStyle = true;
        }

        if (colState.cellFontWeight !== undefined) {
          cellStyleRules += `font-weight: ${colState.cellFontWeight}; `;
          hasCellStyle = true;
        }

        if (colState.cellFontStyle !== undefined) {
          cellStyleRules += `font-style: ${colState.cellFontStyle}; `;
          hasCellStyle = true;
        }

        // Add cell border styles
        if (colState.cellBorderTop && colState.cellBorderTop.style !== 'none') {
          cellStyleRules += `border-top: ${colState.cellBorderTop.width || '1px'} ${colState.cellBorderTop.style || 'solid'} ${colState.cellBorderTop.color || '#dddddd'}; `;
          hasCellStyle = true;
        }

        if (colState.cellBorderRight && colState.cellBorderRight.style !== 'none') {
          cellStyleRules += `border-right: ${colState.cellBorderRight.width || '1px'} ${colState.cellBorderRight.style || 'solid'} ${colState.cellBorderRight.color || '#dddddd'}; `;
          hasCellStyle = true;
        }

        if (colState.cellBorderBottom && colState.cellBorderBottom.style !== 'none') {
          cellStyleRules += `border-bottom: ${colState.cellBorderBottom.width || '1px'} ${colState.cellBorderBottom.style || 'solid'} ${colState.cellBorderBottom.color || '#dddddd'}; `;
          hasCellStyle = true;
        }

        if (colState.cellBorderLeft && colState.cellBorderLeft.style !== 'none') {
          cellStyleRules += `border-left: ${colState.cellBorderLeft.width || '1px'} ${colState.cellBorderLeft.style || 'solid'} ${colState.cellBorderLeft.color || '#dddddd'}; `;
          hasCellStyle = true;
        }

        // Only set cellClass if we have properties to apply
        if (hasCellStyle) {
          // Add the CSS style to the document
          const styleId = `cell-style-${colState.colId}`;
          let styleElement = document.getElementById(styleId) as HTMLStyleElement;
          ...
          styleElement.textContent = `.${cellStyleClass} { ${cellStyleRules} }`;

          // Set the cellClass property
          updatedColDef.cellClass = cellStyleClass;
        } else {
          // If no cell style properties are defined, remove the cellClass property
          delete updatedColDef.cellClass;
        }
